...
def is_on_cooldown(user_id: str) -> bool:
    ...
    if last is None:
        return False
    return (time.time() - last) < REPLY_COOLDOWN_SECONDS
...
def build_dm_message(group: str) -> dict:
    ...
    (см. DM_TEXTS в config.py — свой текст и своя ссылка на каждую тему).
...
@app.post("/alerts/uptime")
async def uptime_alert(request: Request):
    """
    Принимает webhook-оповещение от UptimeRobot при падении сервиса и
    пересылает его вам в Telegram через Bot API вашего существующего бота.
    Настройте в UptimeRobot Webhook Alert Contact с URL вида:
    https://ваш-сервис.onrender.com/alerts/uptime?secret=ВАШ_СЕКРЕТ
    """
    secret = request.query_params.get("secret")
    if not ALERT_WEBHOOK_SECRET or secret != ALERT_WEBHOOK_SECRET:
        logger.warning("Отклонён запрос на /alerts/uptime — неверный или отсутствующий secret")
        return Response(status_code=403)

    body = await request.body()
    alert_text = body.decode("utf-8", errors="replace") or "UptimeRobot: сервис недоступен"
    ...
    async with httpx.AsyncClient() as client:
        ...
        if resp.status_code != 200:
            ...
            logger.info("Алерт от UptimeRobot переслан в Telegram")
