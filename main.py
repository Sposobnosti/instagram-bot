...
async def handle_direct_message(event: dict):
    ...
    await send_direct_message(sender_id, build_dm_message())
...
def build_dm_reply(incoming_text: str) -> str:
    ...
    Оставлено для совместимости; фактический текст берётся напрямую
    в build_dm_message().
    """
    return AUTO_REPLY_DM_TEXT


def build_dm_message() -> dict:
    """
    Собирает тело сообщения для Direct. Button Template (кнопка вместо
    текстовой ссылки) не рендерится для Instagram API с Instagram Login —
    Instagram показывает его как обычный текст без кнопки. Поэтому
    используется простой текст со ссылкой — Instagram сам делает такие
    ссылки кликабельными в Direct.
    """
    return {"text": AUTO_REPLY_DM_TEXT}
...
async def handle_comment(value: dict):
    ...
    await send_private_reply_to_comment(comment_id, build_dm_message())
